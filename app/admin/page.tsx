...
/* ═══════════════════════════════════════════════
   PRODUCTS SECTION
═══════════════════════════════════════════════ */
const ICON_OPTIONS = ["0—Solar","1—Bateria","2—Quadro Elétrico","3—UPS","4—Transformador","5—Veículo Elétrico"];
...
function ProductsSection({ showToast }: { showToast: (m: string, t?: "ok" | "err") => void }) {
  const { items, loading, saving, deleting, save, toggle, remove } = useCrud<Product>("products", showToast);
  const [modal,    setModal]    = useState(false);
  ...
  const openNew = () => { setEditItem(null); setForm(EMPTY_PRODUCT); setModal(true); };
  const openEdit = (p: Product) => { setEditItem(p); setForm({ ...p, specsStr: p.specs.join("\n"), brandsStr: p.brands.join("\n") }); setModal(true); };
  const handleSave = async () => {
    const { specsStr, brandsStr, ...rest } = form;
    const payload = { ...rest, specs: specsStr.split("\n").map(x => x.trim()).filter(Boolean), brands: brandsStr.split("\n").map(x => x.trim()).filter(Boolean) };
    const ok = await save(payload as Omit<Product,"id">, editItem?.id, "Produto");
    if (ok) { setModal(false); setEditItem(null); }
  };
  ...
  return (
    <div>
      ...
      {loading ? <Spinner/> : items.length === 0 ? (
        <EmptyState label="Nenhum produto criado" hint="Crie o primeiro produto" onAdd={openNew}/>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          {items.map((p, i) => (
            <div key={p.id} style={{ ...rowCard, opacity: p.active ? 1 : .65, borderColor: p.active ? "#dde8ea" : "#f0d8d8" }}>
              <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 4, background: p.color, borderRadius: "4px 0 0 4px" }}/>
              <div style={{ paddingLeft: 18, display: "flex", flexDirection: "column", gap: 3, flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                  <span style={{ fontSize: 13, fontWeight: 800, color: "#0a1c1e" }}>{p.name}</span>
                  <StatusBadge active={p.active}/>
                  <span style={{ fontSize: 10, color: "#9bbbbe" }}>/{p.slug} #{i+1}</span>
                </div>
                <p style={{ fontSize: 11, color: "#6a9598", margin: 0, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 1, WebkitBoxOrient: "vertical" as const }}>{p.desc}</p>
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" as const }}>
                  {p.brands.map(b => <span key={b} style={{ fontSize: 9, fontWeight: 700, color: "#095b66", background: "#e8f7f9", borderRadius: 4, padding: "1px 6px" }}>{b}</span>)}
                </div>
              </div>
              <div style={{ paddingRight: 4 }}>
                <RowActions active={p.active} onToggle={() => toggle(p, "Produto")} onEdit={() => openEdit(p)} onDelete={() => setDel(p)}/>
              </div>
            </div>
          ))}
        </div>
      ...
      {modal && (
        <ModalShell
          ...
          footer={
            <>
              <button onClick={() => { setModal(false); setEditItem(null); }} style={cancelBtn}>Cancelar</button>
              <button onClick={handleSave} disabled={!form.name || !form.slug || saving} style={{ ...primaryBtn, background: form.name && form.slug && !saving ? "#095b66" : "#b0c8ca", cursor: form.name && form.slug && !saving ? "pointer" : "not-allowed" }}>
        ...
        >
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <label style={{ display: "flex", flexDirection: "column", gap: 3 }}><span style={lbl}>Nome</span><input style={inpStyle} value={form.name} onChange={e => set("name", e.target.value)} placeholder="Ex: Sistemas de Energia Solar"/></label>
            <label style={{ display: "flex", flexDirection: "column", gap: 3 }}><span style={lbl}>Slug (URL)</span><input style={inpStyle} value={form.slug} onChange={e => set("slug", e.target.value.toLowerCase().replace(/\s+/g,"-"))} placeholder="Ex: solar"/></label>
          </div>
          <label style={{ display: "flex", flexDirection: "column", gap: 3 }}><span style={lbl}>Descrição</span><textarea style={{ ...inpStyle, minHeight: 60, resize: "vertical" }} value={form.desc} onChange={e => set("desc", e.target.value)} placeholder="Texto descritivo..."/></label>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <label style={{ display: "flex", flexDirection: "column", gap: 3 }}><span style={lbl}>Especificações (uma por linha)</span><textarea style={{ ...inpStyle, minHeight: 70, resize: "vertical" }} value={form.specsStr} onChange={e => set("specsStr", e.target.value)} placeholder={"+50 MW instalados\nResidencial · Industrial"}/></label>
            <label style={{ display: "flex", flexDirection: "column", gap: 3 }}><span style={lbl}>Marcas (uma por linha)</span><textarea style={{ ...inpStyle, minHeight: 70, resize: "vertical" }} value={form.brandsStr} onChange={e => set("brandsStr", e.target.value)} placeholder={"Huawei FusionSolar\nSMA"}/></label>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#f8fbfc", border: "1.5px solid #dde8ea", borderRadius: 10, padding: "10px 14px" }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#0a1c1e" }}>Produto activo</span>
            <Toggle on={form.active} onToggle={() => set("active", !form.active)}/>
          </div>
        </ModalShell>
      )}
      {del && <ConfirmDeleteModal title={del.name} hint={`/${del.slug}`} onConfirm={async () => { const ok = await remove(del.id, "Produto"); if (ok) setDel(null); }} onClose={() => setDel(null)} loading={deleting}/>}
    </div>
  );
}
