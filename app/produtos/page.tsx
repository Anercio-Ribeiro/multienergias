...
//         el.style.boxShadow   = "0 8px 32px rgba(9,91,102,.12)";
//         el.style.transform   = "translateY(-3px)";
//       }}
//       onMouseLeave={e => {
//         const el = e.currentTarget as HTMLDivElement;
//         el.style.borderColor = "#dde8ea";
...
//             opacity: 0.18, lineHeight: 1, userSelect: "none",
...
//         {/* Name */}
//         <h3 style={{
//           fontSize: 14, fontWeight: 800, color: "#0a1c1e",
...
//         {/* Desc */}
//         <p style={{
//           fontSize: 12.5, color: "#5a8285", lineHeight: 1.65,
...
//           paddingTop: 12, borderTop: "1px solid #edf3f4",
//         }}>
//           <div>
//             <div style={{ fontSize: 10, color: "#9bbbbe", fontWeight: 600, marginBottom: 2 }}>
//               {p.specs?.[0] ? "Especificação" : "Categoria"}
//             </div>
//             <div style={{ fontSize: 12, fontWeight: 800, color: accent }}>
//               {p.specs?.[0] || p.category}
//             </div>
//           </div>
//           {p.brands && p.brands.length > 0 && (
//             <div style={{ textAlign: "right" }}>
//               <div style={{ fontSize: 10, color: "#9bbbbe", fontWeight: 600, marginBottom: 2 }}>Marca</div>
//               <div style={{ fontSize: 11, fontWeight: 700, color: "#1a2c2e" }}>{p.brands[0]}</div>
//             </div>
//           )}
//         </div>
//       </div>
...
//     const hide = () => {
//       setLoaderFade(true);
//       setTimeout(() => setLoaderVis(false), 480);
//     };
//     if (document.readyState === "complete") { setTimeout(hide, 600); }
//     else { window.addEventListener("load", () => setTimeout(hide, 600), { once: true }); }
//   }, []);
...
//           lightColor: p.lightColor || "#e8f7f9",
//           color:      p.color      || "#095b66",
//         }));
//         setProducts(safe);
//       })
//       .catch(console.error)
...
//   /* Derived filter options from API data */
...
//     const bs = Array.from(new Set(products.flatMap(p => p.brands).filter(Boolean)));
//     return ["Todas as Marcas", ...bs];
//   }, [products]);
...
//       const matchCat    = category === "Todos" || p.category === category;
//       const matchBrand  = brand === "Todas as Marcas" || p.brands.includes(brand);
//       const q           = search.toLowerCase();
//       const matchSearch = !q
...
//         .btn-teal { background:#095b66; color:#fff; border:none; border-radius:6px; padding:13px 28px; font-family:'Montserrat',sans-serif; font-size:12px; font-weight:700; letter-spacing:.07em; text-transform:uppercase; cursor:pointer; transition:all .25s; display:inline-flex; align-items:center; gap:8px; }
//         .btn-teal:hover { background:#0a7a89; transform:translateY(-2px); }
//         @media (max-width:900px) { .products-grid { grid-template-columns:repeat(2,1fr)!important; } .sp { padding-left:22px!important; padding-right:22px!important; } }
//         @media (max-width:540px) { .products-grid { grid-template-columns:1fr!important; } .filters-scroll { overflow-x:auto; padding-bottom:8px; } }
//       `}</style>

//       {/* ── HERO BAND ── */}
//       <div style={{ background:"linear-gradient(135deg,#095b66 0%,#0a7a89 100%)", padding:"120px 80px 48px", position:"relative", overflow:"hidden" }}>
//         <div style={{ position:"absolute", right:-40, top:-40, width:280, height:280, borderRadius:"50%", background:"rgba(255,255,255,.05)" }}/>
//         <div style={{ position:"absolute", right:80, bottom:-60, width:200, height:200, borderRadius:"50%", background:"rgba(255,255,255,.04)" }}/>
//         <div className="sp" style={{ maxWidth:1280, margin:"0 auto", position:"relative", zIndex:1 }}>
//           <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
...
//             {loading
//               ? "A carregar produtos…"
//               : `${products.length} produtos de energia solar, armazenamento, proteção elétrica e mobilidade.`
//             }
//           </p>
//         </div>
//       </div>
...
//         {/* Category chips — derived from API */}
//         <div className="filters-scroll" style={{ display:"flex", gap:8, marginBottom:36, flexWrap:"wrap" }}>
//           {categories.map(cat => (
//             <button key={cat} className={`filter-chip ${category === cat ? "active" : ""}`}
//               onClick={() => handleFilter(cat)}>
...
//               <div key={i} style={{ borderRadius:16, overflow:"hidden", border:"1.5px solid #dde8ea", background:"#fff" }}>
//                 <div style={{ height:160, background:"linear-gradient(90deg,#f0f4f5 25%,#e6eef0 50%,#f0f4f5 75%)", backgroundSize:"200% 100%", animation:"ml-shimmer 1.4s ease-in-out infinite" }}/>
//                 <div style={{ padding:"18px 20px" }}>
//                   <div style={{ height:14, borderRadius:6, background:"#edf3f4", marginBottom:10, width:"70%" }}/>
//                   <div style={{ height:11, borderRadius:6, background:"#edf3f4", marginBottom:6, width:"90%" }}/>
//                   <div style={{ height:11, borderRadius:6, background:"#edf3f4", width:"55%" }}/>
//                 </div>
//               </div>
//             ))}
//           </div>
...
//         {/* Pagination */}
//         {totalPages > 1 && (
//           <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6, marginBottom:44 }}>
//             <button className="page-btn" disabled={page === 1} onClick={() => setPage(p => p - 1)}>
//               <svg viewBox="0 0 16 16" fill="none" width="12" height="12"><path d="M10 12L6 8l4-4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>
//             </button>
//             {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
...
//         {/* CTA */}
//         <div style={{ marginTop:48, background:"linear-gradient(135deg,#095b66,#0a7a89)", borderRadius:20, padding:"44px 52px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:24, flexWrap:"wrap" }}>
//           <div>
//             <h2 style={{ fontSize:"clamp(20px,2.5vw,32px)", fontWeight:900, color:"#fff", marginBottom:8 }}>
//               Não encontrou o que procura?
//             </h2>
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          background: "rgba(6,22,26,.72)", backdropFilter: "blur(6px)",
        }}
      />
      {/* Panel */}
      <div style={{
        ...
        <div style={{
          ...
          maxWidth: 860, maxHeight: "90vh", overflow: "auto",
          boxShadow: "0 32px 80px rgba(6,22,26,.3)",
        }}>

          {/* ── Header ── */}
          <div style={{
            background: `linear-gradient(135deg, ${accent}, ${accent}cc)`,
            padding: "36px 40px 32px", borderRadius: "20px 20px 0 0",
            position: "relative",
          }}>
            <div style={{
              position: "absolute", inset: 0,
              backgroundImage: "linear-gradient(rgba(255,255,255,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.04) 1px,transparent 1px)",
              backgroundSize: "32px 32px", borderRadius: "20px 20px 0 0",
            }}/>
            <div style={{ position: "relative" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: "rgba(255,255,255,.6)", letterSpacing: ".14em", textTransform: "uppercase" }}>
                  {p.category}
                </span>
                <button
                  onClick={onClose}
              ...
              <h2 style={{ fontSize: "clamp(20px,2.5vw,28px)", fontWeight: 900, color: "#fff", lineHeight: 1.2, marginBottom: 10 }}>
                {p.name}
              </h2>
              {p.brands && p.brands.length > 0 && (
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <svg viewBox="0 0 16 16" fill="none" width="12" height="12">
                    <rect x="2" y="2" width="12" height="12" rx="3" stroke="rgba(255,255,255,.7)" strokeWidth="1.3"/>
                    <path d="M5 8h6M8 5v6" stroke="rgba(255,255,255,.7)" strokeWidth="1.3" strokeLinecap="round"/>
                  </svg>
          ...
          <div style={{ padding: "36px 40px" }}>
              ...
              <div style={{
                ...
                {p.image ? (
                  ...
                  <>
                    ...
                    <span style={{
                      position: "relative", fontSize: 80, fontWeight: 900,
            ...
            {/* Meta cards */}
            {(p.specs?.length > 0 || p.brands?.length > 0) && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 12, marginBottom: 28 }}>
                {p.specs?.slice(0, 3).map((s, i) => (
                  <div key={i} style={{ background: "#f8fbfc", border: "1.5px solid #dde8ea", borderRadius: 10, padding: "14px 16px" }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: accent, letterSpacing: ".12em", textTransform: "uppercase", marginBottom: 5 }}>
                      Especificação {p.specs.length > 1 ? i + 1 : ""}
                    </div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#0a1c1e", lineHeight: 1.4 }}>{s}</div>
                  </div>
                ))}
                {p.brands?.length > 0 && (
                  <div style={{ background: "#f8fbfc", border: "1.5px solid #dde8ea", borderRadius: 10, padding: "14px 16px" }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: accent, letterSpacing: ".12em", textTransform: "uppercase", marginBottom: 5 }}>Marcas</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#0a1c1e", lineHeight: 1.4 }}>{p.brands.join(", ")}</div>
                  </div>
                )}
              </div>
            )}
            ...
            {p.highlights?.length > 0 && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 12, marginBottom: 28 }}>
                {p.highlights.map((h, i) => (
                  <div key={i} style={{ background: bg, borderRadius: 10, padding: "14px 16px", border: `1.5px solid ${accent}22` }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: accent, letterSpacing: ".12em", textTransform: "uppercase", marginBottom: 5 }}>{h.label}</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#0a1c1e" }}>{h.value}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            {(p.desc || p.longDesc) && (
              <div style={{ marginBottom: 28 }}>
                {(p.longDesc || p.desc).split("\n\n").map((para, i) => (
                  <p key={i} style={{ fontSize: 14.5, color: "#4a7275", lineHeight: 1.8, marginBottom: 12 }}>{para}</p>
                ))}
              </div>
            )}
            ...
            {/* CTA footer */}
            <div style={{
              marginTop: 32, paddingTop: 24, borderTop: "1.5px solid #edf3f4",
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          position: "absolute", bottom: 10, left: 12,
          background: "rgba(255,255,255,.92)", borderRadius: 99, padding: "3px 10px",
          fontSize: 10, fontWeight: 700, color: accent,
          letterSpacing: ".08em", textTransform: "uppercase",
        }}>
          {p.category}
        </div>
        {/* "Ver detalhes" hint on hover handled by card hover */}
      </div>
      ...
      <div style={{ padding: "18px 20px 20px", display: "flex", flexDirection: "column", flex: 1 }}>
        ...
        {/* Ver detalhes */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 4, marginTop: 12, fontSize: 11.5, fontWeight: 700, color: accent }}>
          Ver detalhes
...
export default function ProdutosPage() {
  ...
  const brandOptions = useMemo(() => ["Todas as Marcas", ...Array.from(new Set(products.flatMap(p => p.brands).filter(Boolean)))], [products]);

  const filtered = useMemo(() => products.filter(p => {
    const matchCat    = category === "Todos" || p.category === category;
    const matchBrand  = brand === "Todas as Marcas" || p.brands.includes(brand);
    const q           = search.toLowerCase();
    const matchSearch = !q || p.name.toLowerCase().includes(q) || p.desc.toLowerCase().includes(q) || p.brands.join(" ").toLowerCase().includes(q) || p.category.toLowerCase().includes(q);
    return matchCat && matchBrand && matchSearch;
  }), [products, search, category, brand]);
    ...
    <div style={{ fontFamily: "'Montserrat', sans-serif", background: "#fff", color: "#1a2c2e", minHeight: "100vh" }}>
      ...
      {loaderVis && (
        <div style={{ position:"fixed",inset:0,zIndex:9999,background:"#095b66",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:20,transition:"opacity .48s cubic-bezier(.4,0,.2,1)",opacity:loaderFade?0:1,pointerEvents:loaderFade?"none":"all" }}>
          <div style={{ display:"flex",alignItems:"center",gap:10,animation:"ml-pulse 1.6s ease-in-out infinite" }}>
            ...
            <span style={{ fontSize:18,fontWeight:900,color:"#fff",letterSpacing:".04em" }}>
              MULTI<span style={{ color:"rgba(255,255,255,.5)" }}>ENERGIA</span>
            </span>
          </div>
          <div style={{ width:180,height:3,borderRadius:99,background:"rgba(255,255,255,.15)",overflow:"hidden",position:"relative" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#095b66 0%,#0a7a89 100%)", padding:"120px 80px 48px", position:"relative", overflow:"hidden" }}>
        ...
        <div className="sp" style={{ maxWidth:1280, margin:"0 auto", position:"relative", zIndex:1 }}>
          ...
          <p style={{ fontSize:15, color:"rgba(255,255,255,.65)", maxWidth:480, lineHeight:1.75 }}>
            {loading ? "A carregar produtos…" : `${products.length} produtos de energia solar, armazenamento, proteção elétrica e mobilidade.`}
          </p>
        </div>
      </div>
      ...
      <div className="sp" style={{ maxWidth:1280, margin:"0 auto", padding:"48px 80px 96px" }}>
        ...
        <div style={{ display:"flex", gap:12, marginBottom:24, flexWrap:"wrap", alignItems:"center" }}>
          <div style={{ position:"relative", flex:"1 1 280px", minWidth:220 }}>
            <svg viewBox="0 0 24 24" fill="none" width="16" height="16" style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }}>
              <circle cx="11" cy="11" r="7" stroke="#9bbbbe" strokeWidth="2"/>
              <path d="M16.5 16.5l4 4" stroke="#9bbbbe" strokeWidth="2" strokeLinecap="round"/>
            </svg>
            <input className="search-input" placeholder="Pesquisar produtos, marcas…"
          ...
          <div style={{ position:"relative" }}>
            ...
            <svg viewBox="0 0 24 24" fill="none" width="14" height="14" style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }}>
              <path d="M6 9l6 6 6-6" stroke="#6a9598" strokeWidth="2" strokeLinecap="round"/>
            </svg>
          </div>
          <div style={{ fontSize:12, fontWeight:600, color:"#9bbbbe", marginLeft:"auto" }}>
        ...
        <div className="filters-scroll" style={{ display:"flex", gap:8, marginBottom:36, flexWrap:"wrap" }}>
          {categories.map(cat => (
            <button key={cat} className={`filter-chip ${category === cat ? "active" : ""}`} onClick={() => handleFilter(cat)}>
              {cat}
            </button>
          ))}
        </div>
        ...
        {!loading && paginated.length > 0 && (
          <div className="products-grid" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:18, marginBottom:44 }}>
            {paginated.map((p, i) => (
              <ProductCard key={p.id} p={p} index={i} onClick={() => setSelected(p)} />
            ))}
          </div>
        )}
        ...
        <div style={{ marginTop:48, background:"linear-gradient(135deg,#095b66,#0a7a89)", borderRadius:20, padding:"44px 52px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:24, flexWrap:"wrap" }}>
          <div>
            <h2 style={{ fontSize:"clamp(20px,2.5vw,32px)", fontWeight:900, color:"#fff", marginBottom:8 }}>Não encontrou o que procura?</h2>
            <p style={{ fontSize:14, color:"rgba(255,255,255,.65)", maxWidth:400, lineHeight:1.75 }}>
              A nossa equipa técnica pode dimensionar uma solução personalizada para o seu projeto.
            </p>
          </div>
          <div style={{ display:"flex", gap:12 }}>
            <Link href="/#contacto" style={{ background:"#fff", color:"#095b66", borderRadius:6, padding:"13px 28px", fontSize:12, fontWeight:700, letterSpacing:".07em", textTransform:"uppercase", display:"inline-flex", alignItems:"center", gap:8, textDecoration:"none" }}>
              Fale Connosco
            </Link>
            <a href="https://wa.me/244933153362" target="_blank" rel="noopener noreferrer"
              style={{ background:"rgba(255,255,255,.12)", color:"#fff", border:"2px solid rgba(255,255,255,.35)", borderRadius:6, padding:"11px 22px", fontSize:12, fontWeight:700, letterSpacing:".07em", textTransform:"uppercase", display:"inline-flex", alignItems:"center", gap:8, textDecoration:"none" }}>
